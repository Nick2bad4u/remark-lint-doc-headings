# Contributing to remark-lint-doc-headings

Thanks for your interest in contributing.

This repository contains a remark-lint rule for enforcing ordered documentation
headings in selected Markdown files.
...
- `src/**`: do **not** commit `console.*` or `debugger` statements.
- `scripts/**`: `console.log`/`console.warn`/`console.error` are allowed for
  maintenance progress and diagnostics.
...
```text
.
├── src/                  # Remark plugin and exported option types
...
- `:sparkles: feat(options): add custom detail heading parents`
- `:bug: fix(plugin): respect excluded rule docs`
- `:memo: docs: clarify heading order options`
